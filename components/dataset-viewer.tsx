...
export const DatasetViewer = () => {
  ...
  useEffect(() => {
    ...
    const checkAvatarReady = () => {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (hasPlaySign || hasAvatar) {
          console.log('[Dataset] Avatar detected! playSign:', hasPlaySign, 'canvas:', !!avatarCanvas, 'iframe:', !!avatarIframe)
    ...
    const interval = setInterval(() => {
      attempts++
      console.log(`[Dataset] Checking avatar readiness... attempt ${attempts}/${maxAttempts}`)
      if (checkAvatarReady() || attempts >= maxAttempts) {
        if (attempts >= maxAttempts) {
          console.warn('[Dataset] Avatar check timed out after', maxAttempts * 300, 'ms')
        }
